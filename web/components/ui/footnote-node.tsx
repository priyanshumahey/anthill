  ...
  return (
    ...
    >
      ...
      <HoverCard open={hoverOpen} onOpenChange={setHoverOpen} openDelay={150}>
        <HoverCardTrigger render={<button type="button" className={cn(
    ...
    >
      <div contentEditable={false}>
        {isDuplicateDefinition ? (
          ...
          >
            <PopoverAnchor render={<button type="button" aria-expanded={
                                            hasMultipleReferences ? referencePickerOpen : undefined
                                          } aria-haspopup={hasMultipleReferences ? 'dialog' : undefined} aria-label={`Back to reference ${identifier}`} className="min-w-3 cursor-pointer rounded-xs text-muted-foreground text-xs tabular-nums underline-offset-2 hover:text-foreground focus:ring-2 focus:ring-ring focus:ring-offset-1" onClick={(event) => {
                                            event.preventDefault();
                                            event.stopPropagation();
                                          }} onMouseDown={(event) => {
                                            event.preventDefault();
