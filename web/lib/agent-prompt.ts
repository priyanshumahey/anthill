/**
 * Builds the copy-paste prompt that an external agent (Claude Code,
 * Copilot, ChatGPT, …) can ingest to start mutating an Anthill document
 * over the agent bridge.
 *
 * The Markdown source-of-truth lives at collab/AGENT_PROMPT.md; this
 * function produces the same body inline so the editor can render it
...
export interface AgentPromptParams {
  bridgeUrl: string;
  documentId: string;
  /** When true, includes auth-token instructions. */
...
export function buildAgentPrompt({
  ...
  const tokenLine = hasSharedSecret
    ? '- X-Agent-Token: <shared secret given to you by the operator>  # required'
    : '- X-Agent-Token: <shared secret>                   # required ONLY if the bridge enforces it (this instance does not)';
...
1. GET ${bridgeUrl}/.well-known/agent.json
   Discovery. Lists supported ops and block types.

2. GET ${bridgeUrl}/documents/${documentId}/snapshot
...
3. GET ${bridgeUrl}/documents/${documentId}/state
...
4. POST ${bridgeUrl}/documents/${documentId}/edit
...
curl -sS -X POST ${bridgeUrl}/documents/${documentId}/edit \\
